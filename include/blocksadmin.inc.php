...
if (\Xmf\Request::hasVar('previewblock', 'POST')) {
    ...
    $bmodule    = \Xmf\Request::getArray('bmodule', [], 'POST'); //TODO check if it is array
    $btitle     = \Xmf\Request::getInt('btitle', '', 'POST');
    $bcontent   = \Xmf\Request::getText('bcontent', '', 'POST');
...
if ('order' === $op) {
    ...
    if (\Xmf\Request::hasVar('side', 'POST')) {
        $side = \Xmf\Request::getInt('side', 0, 'POST');
    }
    //  if ( !empty($_POST['weight']) ) { $weight = $_POST['weight']; }
    if (\Xmf\Request::hasVar('visible', 'POST')) {
        $visible = \Xmf\Request::getInt('visible', 0, 'POST');
    }
    ...
    //    }
    if (\Xmf\Request::hasVar('bid', 'POST')) {
        $bid = \Xmf\Request::getInt('bid', [], 'POST');
    }
    // GIJ start
    foreach (array_keys($bid) as $i) {
        ...
        $bmodule1 = \Xmf\Request::hasVar('bmodule'[$i], 'POST') && is_array(\Xmf\Request::getArray('bmodule'[$i], '', 'POST')) ? \Xmf\Request::getArray('bmodule'[$i], '', 'POST') : [-1];
...
if ('delete_ok' === $op) {
    ...
    if ('' != $myblock->getVar('template') && !defined('XOOPS_ORETEKI')) {
        ...
        if (is_array($btemplate) && count($btemplate) > 0) {
